...
import { transformBeanFactoryMethod } from '../../../atomic-mode/transformers/transformBeanFactoryMethod';
import { isDecoratorFromLibrary } from '../../../decorator-processor/isDecoratorFromLibrary';
import { getConfigurationStaticInitBlock } from './getConfigurationStaticInitBlock';
import { getConfigurationInitProperty } from './getConfigurationInitProperty';
...
export const transformConfigurationClass = (configuration: Configuration): ts.ClassDeclaration => {
  ...
  return factory.updateClassDeclaration(
    classDeclaration,
    classDeclaration.modifiers?.filter(modifier => !isDecoratorFromLibrary(modifier, undefined)),
    classDeclaration.name,
    classDeclaration.typeParameters,
    classDeclaration.heritageClauses,
    [
      ...updatedMembers,
      getConfigurationStaticInitBlock(),
      getConfigurationInitProperty(configuration),
    ]
  );
};
