...
export const verifyTSVersion = () => {
  ...
  if (!semver.satisfies(ts.version, cachedTSVersion)) {
    throw new Error(chalk.red(`${chalk.bold(CONSTANTS.libraryName)} works correctly with typescript version "${chalk.bold(cachedTSVersion)}", you have version "${chalk.bold(ts.version)}", you can disable this error check by setting "unsafeTSVersion" flag in ${chalk.bold('.clawjectrc')} config file.`));
  }
};
