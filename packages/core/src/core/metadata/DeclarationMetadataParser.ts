...
export class DeclarationMetadataParser {
  static parse(classDeclaration: ts.ClassDeclaration): DeclarationMetadata | AbstractCompilationMessage | null {
    ...
    const declarationTypeReferenceNode = metadataProperty.type;

    if (!declarationTypeReferenceNode) {
    ...
    const typeSymbol = Context.typeChecker.getTypeFromTypeNode(declarationTypeReferenceNode).getSymbol();
    ...
    if (!Context.ts.isTypeLiteralNode(typeDeclaration)) {
      return new CorruptedMetadataError(
        'Compiled metadata property must have type literal node.',
        classDeclaration,
        null,
    ...
    return this.parseTypeNode(typeDeclaration);
  }

  static parseTypeNode(node: ts.TypeNode): any | null {
    switch (true) {
