...
It occurs when the type of the bean is not supported by Clawject, here defined restricted [Bean types](/docs/04-base-concepts/06-bean/02-bean-types.mdx). Example:
```ts
class MyContext extends CatContext {
...
to get [`InitializedContext`](/docs/04-base-concepts/04-initialized-context.mdx) object
from [`ContainerManager`](/docs/04-base-concepts/03-container-manager.mdx).
Example:
```ts
class MyContext extends CatContext {}
...
### BeanNotFoundError
It occurs when the bean that is requested is not found in the context. Example:
```ts
interface IMyContext {
  cat: Cat;
}
...
**Solution:** Do not return primitive values from beans that are decorated with scope that defines `useProxy` method that is returned `true`.
