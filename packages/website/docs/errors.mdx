...
- Check a topic about [how dependencies are resolved](/docs/overview/bean.mdx#how-dependencies-are-resolved)
- If more than one matching dependency is defined - rename parameter to match one of the resolved dependencies by name.
- Specify primary bean for the dependency with [`@Primary`](/docs/overview/primary.mdx) decorator.

### CT5: BeanCandidateNotFoundError
It occurs when the bean candidate for the specific dependency could not be resolved.
- When declaring bean with [@Bean decorator](/docs/overview/bean.mdx#bean-decorator) - error will
  appear on the corresponding parameter of the **factory-function**.
- When declaring bean with [Bean function](/docs/overview/bean.mdx#bean-function) - error will
...
It occurs when the type of the bean is not supported by Clawject, here defined restricted [Bean types](/docs/overview/bean.mdx#bean-types). Example:
```ts
class MyContext extends CatContext {
...
It occurs when the dependencies of class constructor could not be resolved. This error occurs only when using [Bean function](/docs/overview/bean.mdx#bean-function). Examples:
...
**Solution:** Use **factory-method** with [@Bean decorator](/docs/overview/bean.mdx#bean-decorator)
...
- Using `@PostConstruct` decorator on method with arguments outside CatContext class:
  ...
  ```
- Using `@Embedded` decorator with bean declared with [Bean function](/docs/overview/bean.mdx#bean-function):
  ```ts
  class MyContext extends CatContext {
...
It occurs when few beans have the same name (class member name or name defined via [`@Qualifier decorator`](/docs/overview/qualifier.mdx)). Example:
```ts
class MyContext extends CatContext {
  cat = Bean(Cat);
