...
- Check a topic about [how dependencies are resolved](/docs/overview/bean#how-dependencies-are-resolved)
- If more than one matching dependency is defined - rename parameter to match one of the resolved dependencies by name.
- Specify primary bean for the dependency with [`@Primary`](/docs/overview/primary) decorator.

#### CE5: BeanCandidateNotFoundError
It occurs when the bean candidate for the specific dependency could not be resolved.
- When declaring bean with [@Bean decorator](/docs/overview/bean#bean-decorator) - error will
  appear on the corresponding parameter of the **factory-function**.
- When declaring bean with [Bean function](/docs/overview/bean#bean-function) - error will
...
It occurs when the type of the bean is not supported by Clawject, here defined restricted [Bean types](/docs/overview/bean#bean-types). Example:
```ts
@ClawjectApplication
class Application {
...
- Using `@PostConstruct` decorator on method with arguments outside CatContext class:
  ...
  ```
- Using `@Embedded` decorator with bean declared with [Bean function](/docs/overview/bean#bean-function):
  ```ts
  @ClawjectApplication
  class Application {
...
It occurs when you try to import class that is not annotated with `@Configuration` or `@ClawjectApplication`. Example:
```ts
class FooConfiguration {}

@ClawjectApplication
class Application {
  foo = Import(FooConfiguration);
}
```

**Solution:** Add `@Configuration` or `@ClawjectApplication` annotation to the imported class.

#### CE14: DuplicateNameError
It occurs when few beans have the same name (class member name or name defined via [`@Qualifier decorator`](/docs/overview/qualifier)). Example:
```ts
@ClawjectApplication
class Application {
  cat = Bean(Cat);
