...
When you're decorating property that holds **arrow function** or **method** -
it's your responsibility to provide return value for bean,
if it's class - it should be instantiated, if it's some dynamic value — it should be provided.

You can provide bean dependencies via function arguments by specifying an argument type.

```typescript title="Declaring method and arrow function beans"
class Foo {}
...
class MyContext extends CatContext {
    ...
    @Bean fooClassName = (foo: Foo): string => foo.constructor.name;
}
```
...
```typescript title="Declaring bean without dependencies"
class Foo {}
...
especially when your class has a lot of dependencies because,
if you're adding or removing a dependency, you need to change it in container level as well.
