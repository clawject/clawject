...
### `ContainerManager.registerScope`

This method allows you to register a new [Custom Scope](../05-advanced-concepts/02-custom-scopes.mdx) and [use it](./09-scope.mdx) for beans.

#### Registering a new scope

```ts
import { ContainerManager } from 'clawject';
import { HTTPRequestScope } from './HTTPRequestScope';

ContainerManager.registerScope('http-request', new HTTPRequestScope());
```
