...
:::warning
Be careful with asynchronous beans which **scope** is not **singleton**.
If such a bean is requested **after** application context initialization - it could lead to unexpected behavior.
:::
