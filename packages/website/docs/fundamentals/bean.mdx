...
:::warning Be careful
Asynchronous beans which **scope** is not **singleton** could lead to unexpected behavior.
If such a bean is requested **after** application context initialization - promise could be injected instead of resolved value.
Work in progress to provide a solution for this case.
:::
