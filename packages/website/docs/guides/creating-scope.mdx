...
Out of the box, Clawject provides 2 scopes: `singleton` and `transient`, but sometimes you may need to define your own scopes.
...
is using [AsyncLocalStorage](https://nodejs.org/overview/async_context.html#class-asynclocalstorage)
...
allows us to retrieve `currentRequestId`, and notify listener when http request is finished:
...
One of the first things to consider when implementing a `Scope` class is how you will store
...
The second parameter, scope, is an actual instance of the `Scope` implementation that you wish to register and use.
