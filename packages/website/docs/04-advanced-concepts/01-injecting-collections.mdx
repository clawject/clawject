...
All arrays and collections that are injected are non-singletons, so mutating one of them will not affect other beans.
...
```ts
class MyContext extends CatContext {
    @Bean myCollection: Set<string> = new Set(['foo', 'bar', 'baz']);
    @Bean string1 = 'quux';
    @Bean string2 = 'quuux';
