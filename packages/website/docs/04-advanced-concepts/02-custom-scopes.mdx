...
For example, if you are developing a multi-session system,
you may want to provide a separate instance of a particular bean or set of beans for each session.
Clawject provides a mechanism for creating custom scopes for scenarios such as this.

## Creating a CustomScope Class

In order to create a custom scope, you must implement the `CustomScope` interface.

In the following steps, we will implement http-session scope that 
is using [AsyncLocalStorage](https://nodejs.org/api/async_context.html#class-asynclocalstorage) to retrieve current `http request id`.
...
Finally, let's implement the `dispose` method, that will be called when `http-request` will be finished
