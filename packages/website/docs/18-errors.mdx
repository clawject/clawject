...
- Check a topic about [how dependencies are resolved](/docs/09-bean/03-bean-dependencies.mdx#how-dependencies-are-resolved)
- If more than one matching dependency is defined - rename parameter to match one of the resolved dependencies by name.
- Specify primary bean for the dependency with [`@Primary`](/docs/10-primary.mdx) decorator.

### CT5: BeanCandidateNotFoundError
It occurs when the bean candidate for the specific dependency could not be resolved.
- When declaring bean with [@Bean decorator](/docs/09-bean/01-declare-bean.mdx#using-bean-decorator) - error will
  appear on the corresponding parameter of the **factory-function**.
- When declaring bean with [Bean function](/docs/09-bean/01-declare-bean.mdx#using-bean-function) - error will
...
It occurs when the type of the bean is not supported by Clawject, here defined restricted [Bean types](/docs/09-bean/02-bean-types.mdx). Example:
```ts
class MyContext extends CatContext {
...
It occurs when the dependencies of class constructor could not be resolved. This error occurs only when using [Bean function](/docs/09-bean/01-declare-bean.mdx#using-bean-function). Examples:
...
**Solution:** Use **factory-method** with [@Bean decorator](/docs/09-bean/01-declare-bean.mdx#using-bean-decorator)
...
- Using `@PostConstruct` decorator on method with arguments outside CatContext class:
  ...
  ```
- Using `@Embedded` decorator with bean declared with [Bean function](/docs/09-bean/01-declare-bean.mdx#using-bean-function):
  ```ts
  class MyContext extends CatContext {
...
It occurs when few beans have the same name (class member name or name defined via [`@Qualifier decorator`](/docs/11-qualifier.mdx)). Example:
```ts
class MyContext extends CatContext {
  cat = Bean(Cat);
...
to get [`InitializedContext`](/docs/07-initialized-context.mdx) object
from [`ContainerManager`](/docs/06-container-manager.mdx).
Example:
```ts
class MyContext extends CatContext {}
...
### IllegalAccessError
It occurs when you're trying to access some internal clawject property or method that is not allowed to access.
