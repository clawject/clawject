...
Let's define a simple application with a single bean and try to expose it using [ExposeBeans function](/docs/api/expose-beans.mdx):
...
[@PostConstruct](/docs/api/lifecycle.mdx#postconstruct) and [@PreDestroy](/docs/api/lifecycle.mdx#predestroy) decorators:
